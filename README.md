...
| 要素 | スマホ（< 768px） | タブレット（>= 768px） |
|---|---|---|
| ナビゲーション | 下部タブバー | 左サイドバー（幅 200px） |
| ホーム モード選択 | 縦リスト | 3カラムグリッド |
| 学習モード カード | 縦積み（漢字のみ大きく） | 横並び（漢字 + 例文を同時表示） |
| テスト 4択 | 2×2グリッド | 2×2グリッド（余白ゆったり） |
| 復習一覧 | 1カラム | 2カラム |
| 漢字フォントサイズ | 56px | 72px |
| ベース文字サイズ | 14px | 16px |
...
- 全画面で `useDeviceType()` を使いレイアウトを切り替える
- `StyleSheet.create()` 内でスマホ用スタイルを定義し、タブレット時は上書きする
- タブバーとサイドバーは `_layout.tsx` で切り替える
- フォントサイズは定数ファイルで一元管理する
...
| 画面 | スマホ | タブレット追加仕様 |
|---|---|---|
| オンボーディング① | 言語選択（日本語 / English 2択） | 中央寄せ、選択肢を大きく |
| オンボーディング② | HSK レベル選択（2×3グリッド） | 同左 |
| ホーム | 縦リスト・進捗サマリー | 3カラムグリッド |
| 学習（表面） | 漢字大表示・発音ボタン | 文字さらに大きく |
| 学習（裏面） | 漢字＋ピンイン＋意味＋例文 | 漢字と例文を左右並列表示 |
| テスト | 漢字＋ピンイン＋4択 | 余白ゆったり・文字大きめ |
| 復習一覧 | 誤答カード縦スクロール | 2カラム表示 |
| 結果 | スコア＋間違い一覧 | 中央寄せ・スコア大きく |
| 設定 | 縦リスト | サイドバー選択＋右パネル |

### 誤答バッジの色ルール
- 3回以上: 赤（#FCEBEB / #A32D2D）
- 2回: オレンジ（#FAEEDA / #854F0B）
- 1回: 緑（#E1F5EE / #0F6E56）
